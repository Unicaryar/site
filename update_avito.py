...
def extract_page_data(page_html: str) -> tuple[list[str], str]:
    ...
    # Сначала читаем структурированные данные страницы.
    ...
    # Резервный поиск CDN-ссылок в HTML/JSON страницы.
    patterns = [
    ...
    for url in images:
        ...
        if url in seen:
            continue
        if any(term in url.lower() for term in ("logo", "avatar", "icon", "favicon")):
            continue
...
def download_page_images(
    ...
    # Удаляем только автоматически скачанные изображения текущей карточки.
...
def scrape_avito_page(
    ...
    print(
        ...
        f"скачано фото {len(local_images)}"
    )
...
def main() -> int:
    try:
        ...
        for api_car in api_cars:
            ...
            # Страницу Avito открываем только когда после XML,
            # ручных данных и прошлого каталога не хватает фото/описания.
